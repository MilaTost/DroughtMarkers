...
lambda_value <- 1e-10
method <- "LFMM2"
run_description <- paste0("K", "_", latent_factor_K, "_Randomization")
### Load files -----------------------------------------------------------------
VCF_file <- "all_merged.norm.filtered.vcf"
VCF_file_describ <- "RomanianBeech"
VCF_file_dir <- "/usr/users/mtost/Beech_project/Romanian_Beech_Data/Genotypic_data/"
working_dir <- "/usr/users/mtost/Beech_project/Romanian_Beech_Data/EAA/LFMM/LFMM2_final_run/"
result_dir <- "/usr/users/mtost/Beech_project/Romanian_Beech_Data/EAA/LFMM/LFMM2_final_run/Results_Run6/"
result_dir_BH <- "/usr/users/mtost/Beech_project/Romanian_Beech_Data/EAA/LFMM/LFMM2_final_run/BH_corrected_results/"
input_file_dir <- "/usr/users/mtost/Beech_project/Romanian_Beech_Data/Phenotypic_data/"
lfmm_input_dir <- "/usr/users/mtost/Beech_project/Romanian_Beech_Data/EAA/LFMM/LFMM2_final_run/new_input_dir/"
plot_dir <- "/usr/users/mtost/Beech_project/Romanian_Beech_Data/EAA/LFMM/LFMM2_final_run/Plot_dir_Run6/"
### Load the data sets ---------------------------------------------------------
#### Load the VCF file
...
colnames(new_dt_effect_estimates) <- c("SNP_ID", "Variable", "effect_estimates", "pvalues", "z-scores")
#write.table(new_dt_effect_estimates, paste0(result_dir, Sys.Date(), "LFMM_based_on_", run_description, "_", var_name, ".txt"))
### BH correction --------------------------------------------------------------
dt <- new_dt_effect_estimates
rm(new_dt_effect_estimates)
dt <- dt[-which(str_detect(dt$SNP_ID,"Bhaga_Unplaced")),]
#dt$pvalues_pv <- as.numeric(iconv(dt$pvalues_pv, 'utf-8', 'ascii', sub=''))
#p <- format(dt$pvalues_pv,scientific = FALSE)
p <- as.numeric(dt$pvalues)
cat("P-values:", range(p), "\n")
suppressWarnings(fdr_test <- p.fdr(
  pvalues = p,
  adjust.method = "BH",
  sort.results = FALSE)
)
dt_result <- fdr_test$`Results Matrix`
SNP_ID_split <- str_split(dt$SNP_ID, "_")
SNP_ID_split <- unlist(SNP_ID_split)
SNP_ID_mat <- matrix(data = SNP_ID_split,
                     nrow = length(dt$SNP_ID), ncol = 3, byrow = TRUE)
Chromomsome <- SNP_ID_mat[ ,2]
Position <- SNP_ID_mat[ ,3]
cat(var_name, "is getting processed.", "\n")
new_dt <- cbind(Chromomsome, Position, dt$SNP_ID, dt$Variable, dt_result, dt$effect_estimates)
colnames(new_dt) <- c("Chr", "Pos", "SNP_ID", "Variable", "BH_FDRs", "Adj_p_values", "Raw_pvalues", "Effect_estimates")
#fwrite(new_dt, paste0(result_dir_BH, "BH_corrected_results_", var_name, "_", run_description,".txt"), sep="\t", quote=F)
### How many significant observations ------------------------------------------
p_threshold = 0.001
nr_markers <- length(which(new_dt$BH_FDRs <= p_threshold))
dt_random_LFMM <- cbind(rep_name, p_threshold, var_name, nr_markers)
dt_random_LFMM <- as.data.frame(dt_random_LFMM)
colnames(dt_random_LFMM) <- c("Replication", "p_threshold", "Variable", "Nr_of_sign_markers")
write.table(dt_random_LFMM, paste0(result_dir, Sys.Date(),"_Nr_sign_markers_efter_randomization_", run_description, "_", rep_name,".txt"),
            sep = "\t",
            row.names = FALSE)
cat("For ", rep_name,":", var_name, "->", nr_markers, "were discoverd!", "\n")
q()
